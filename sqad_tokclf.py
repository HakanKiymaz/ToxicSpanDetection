...
print("This is the second step of Sequential Adaption approach\n")
...
f1 = plot_seq_len(data, name='train')
f1.savefig("./results/train_seq_len_sqad_tokclf.png")
maxlen_train = max([len(x) for x in data['bert_tokens']])
print("maxlen_train : ", maxlen_train)

f2 = plot_seq_len(data_test, name='test')
f2.savefig("./results/test_seq_len_sqad_tokclf.png")
...
print(train_y.shape, np.mean(train_y))
...
f3.savefig("./results/batch_losses_sqad_tokclf.png")

print("Change model mode to eval mode...1<")
model.eval()
...
f4.savefig("./results/threshold-f1_score-sqad-tknclf.png")

f5 = plt.figure()
plt.plot(fpr_scores, tpr_scores)
f5.savefig("./results/fpr_scores-tpr_scores-sqad-tknclf.png")

with open('./results/spans-pred_sqad_tknclf.txt', 'w') as out:
    for uid, text_scores in zip(range(len(test_tokens)), get_scores(0.5)[3]):
        out.write(f'{str(uid)}\t{str(text_scores)}\n')


with open('./results/f1_sqad_tknclf.pkl', 'wb') as f:
    pickle.dump(f1_scores, f)
with open('./results/tpr_sqad_tknclf.pkl', 'wb') as f:
    pickle.dump(tpr_scores, f)
with open('./results/fpr_sqad_tknclf.pkl', 'wb') as f:
    pickle.dump(fpr_scores, f)
with open("./result/loss_sqd_tokclf.pkl", "w") as f:
